...
>
> **S1.6 — feature flag للـ legacy ClaudeHandoffModal** ✅. الـ "🤖 Send to Claude" button + الـ modal mount في [`VideoCard.tsx`](dashboard-ui/src/components/VideoCard.tsx) بقوا wrapped في `{SHOW_LEGACY_HANDOFF && ...}` constant. الـ flag = `import.meta.env.VITE_STAGE_ALPHA_LEGACY === '1'`. **Default off** فالزرار مخفي والـ modal مش بيـ render. **للـ revert** لو حصل bug في الـ Stage α panel: `VITE_STAGE_ALPHA_LEGACY=1 npm run dashboard` يرجع الـ legacy workflow فوراً بدون code change. الـ button label بقى "🤖 Send to Claude (legacy)" لما ظاهر عشان واضح إنه fallback. الـ ClaudeHandoffModal.tsx + `handleAnalyze` + `handoffOpen` state يفضلوا موجودين (TypeScript بيـ count الـ conditional JSX كـ usage فمفيش noUnusedLocals errors).
>
...
>
> **Mega-Batch Mode B (Parallel workflow)**. الـ BatchToolbar اكتسب row تانية "🚀 Mega-Batch" فيها 4 زرارات بالتسلسل: **1️⃣ Prep All** → chains `phase1` batch ثم `transcribe` batch sequentially (frontend orchestration فوق الـ existing batch runner). **2️⃣ Send All to Claude** → يفتح MegaHandoffModal بياخد aggregated message من `POST /api/mega/handoff-all` (واحد message فيه كل الفيديوهات + instructions لـ Claude يعملهم في session واحدة بالتتابع). **3️⃣ Render All** → delegates للـ existing bulk render. **4️⃣ Commit Batch** → ينادي `POST /api/mega/commit-batch` اللي بيعمل: (a) upsert feedback/log.json بـ entry لكل فيديو (counts من animation_plan.json)، (b) prepend batch summary block لـ CLAUDE.md Next Up، (c) `git add` specific files (feedback/log.json + CLAUDE.md + src/data/<slug>/ لكل فيديو)، (d) `git commit` بـ auto-generated message فيه الـ ratings + context. الـ commit بيـ validate إن كل الـ videos عندها rating قبل ما يبدأ — الـ button disabled لو فيه rating ناقص. **Mode A (Serial)** اتأجّل لحد ما نشوف Mode B كافي ولا لأ بعد بعض الـ batches الحقيقية.
>
