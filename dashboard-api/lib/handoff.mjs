...
export function buildHandoffMessage(video) {
  ...
  return [
    ...
    '',
    'قواعد مهمة (اقرأ بالترتيب قبل أي تعديل):',
    '1. CLAUDE.md',
    '2. brands/rs/BRAND.md (قواعد البراند الإجبارية)',
    '3. feedback/style_evolution.md (تفضيلات Omar المتراكمة)',
    '4. feedback/reference_library.md (patterns مستخرجة من reels خارجية — فكّر فيهم بعد الـ content analysis، طبّق الـ high-fit فقط (1-2 max per reel)، لو مفيش fit مناسب اشرح ليه باختصار في الـ plan. الـ `avoid` patterns تأكد إن الـ plan ما يعملش-ـها.)',
    '',
