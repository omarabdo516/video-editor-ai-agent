...
> **مبدأ أساسي:** الـ skill دي **dynamic**. الحقائق المتغيرة (قايمة scene types، caption styles، packages، spacing rules، tagline، إلخ) **مش موجودة هنا** — Claude بيقراها كل session من الـ source of truth files المذكورة تحت. لما الـ pipeline يتحدّث، الـ skill تفضل صح من غير تعديل.
...
| هدف | Source of Truth |
|-----|------------------|
| حالة المشروع الحالية + قرارات معمارية + "next up" | `CLAUDE.md` (root) |
...
| قواعد الـ memory الـ user-level | `~/.claude/projects/c--Users-PUZZLE-Documents-Claude-video-editor-ai-agent/memory/MEMORY.md` (auto-loaded) |
...
4. لو تمام:
   ...
   - لو preference متراكم → memory file في `~/.claude/projects/.../memory/` + pointer في `MEMORY.md`
